...
def cluster_bootstrap(by_cat_values, reducer, n=2000):
    ...
    for _ in range(n):
        ...
        for _ in range(len(cats)):
            ...
            if vals:
                pooled.extend(random.choice(vals) for _ in vals)
        if pooled:
...
def main():
    ...
    lines = [
        ...
        r"\begin{table}[t]",
        r"\caption{E5 accuracy with 95\% cluster-bootstrap confidence intervals "
        ...
        r"only ones that license a directional claim.}",
