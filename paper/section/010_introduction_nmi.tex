...
The present paper makes four contributions. First, it defines situation
...
SituationCatch-Bench, and uses matched state conditions and component ablations
to demonstrate that the specified applicability mechanisms are executable and
causally separable. Fourth, it reports an audited, deliberately bounded
three-model Gemini-family intervention with a focused retrieval baseline. Its
null-to-negative aggregate result prevents us from equating an explicit prompt
slot with situation awareness, while its temporal and observer-category pattern
motivates a larger cross-family natural-text test. This claim boundary
distinguishes what we demonstrate from broad language-model generalization.
